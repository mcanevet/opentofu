...
- Provider version constraints now support prerelease versions using the semver -0 suffix convention (e.g., `>= 1.0.0-0` matches `1.0.0-beta.1`). **Note:** this is a behavior change — constraints with a prerelease boundary previously matched only stable releases. ([#XXXX](https://github.com/opentofu/opentofu/pull/XXXX))
